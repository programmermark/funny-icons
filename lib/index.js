...
const config = require(resolve(CWD, "funny-icons.config.js"));
...
async function build() {
  ...
  const icons = readdirSync(SVG_DIR).map((svgName) => {
    ...
    return {
      ...
      pointCode: svgName.slice(1, i).replace("E", "F"),
    };
  });
...
@font-face {
  ...
  src: url("${
    base64
      ? `data:application/font-woff2;charset=utf-8;base64,${Buffer.from(
          woff2
        ).toString("base64")}`
    ...
    url("${
      base64
        ? `data:application/font-woff;charset=utf-8;base64,${woff.toString(
            "base64"
          )}`
...
async function cli() {
  commander
    .command("build")
    .description("Build funny icons from svg")
    .action(build);
